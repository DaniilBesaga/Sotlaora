...
//       try {
//         const response = await authorizedFetch('http://localhost:5221/api/user/orders', { method: 'GET' });
//         if (response.ok) {
...
export default function UserCabinet() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await authorizedFetch('http://localhost:5221/api/client/orders', { method: 'GET' });
        if (response.ok) {
          const data: OrderDTO[] = await response.json();
          setOrdersData(data);
        } else {
          ...
          <section className={styles.contentArea}>
             {displayedOrders.length > 0 ? (
                ...
                <div className={styles.emptyState}>
                   <FileText size={48} className={styles.emptyIconMain} />
                   <h3 className={styles.emptyTitle}>No orders here</h3>
                   <a href='/create-order' className={styles.createOrderBtnLarge}><Plus size={20}/> Create Order</a>
                </div>
             )}
          </section>
        </main>
